...
window.onload=function () {
   ...
   // var key = $.cookie('username');
    ...
    if(asd!=null){
        if(asd.id=="user"){
            repalce(asd);
            let manger2 = new RegExp("manger2.0.html");
            if(manger2.test(window.location.href)){
            	window.location.replace('beforeindex.html')
            }
        }else {
            repalce(asd);
            if($('#top ul:nth-child(2)').children('#hou'))
            $('#top ul:nth-child(2)').append('<li id="hou"><a href="manger2.0.html">后台管理</a></li>')
    ...
    let register = new RegExp("register.html")
